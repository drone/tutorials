...
This guide will walk you through the steps required to build multi-architecture Rust projects and publish the binaries on GitHub. This guide assumes you have a basic understanding of Rust and GitHub.
...
Our goal is to compile the Rust project for three different plattforms: `amd64`, `aarch64` and `armv7`. Our Cross-Compilation Docker image from the previous tutorial supports only the current plattform (`amd64` in this case) and `aarch64`. To add support for `armv7` we need to install a compatible linker and the Rust Standard Library. To do this add the highlighted lines to the `dockerfile` and build it.
...
$ docker run --rm -v <path to local project>:/rust-project -w /rust-project rust-multiarch cargo build
# Build for aarch64
$ docker run --rm -v <path to local project>:/rust-project -w /rust-project rust-multiarch cargo build --target=aarch64-unknown-linux-gnu
# Build for armv7
$ docker run --rm -v <path to local project>:/rust-project -w /rust-project rust-multiarch cargo build --target=armv7-unknown-linux-gnueabihf
```
...
To organize these assets we create a new directory `assets` and copy (and rename) them:
...
To create a GitHub release, we first need to [commit our project to GitHub](https://help.github.com/en/articles/creating-a-repository-on-github) and create a git tag for the current version:
...
To create a release now from this tag we could use the GitHub user interface or the GitHub API. In this tutorial we will use the API with `curl`.

{{< alert info >}}
To use the GitHub API you need [a token for authorization](https://github.com/settings/tokens). If you don't have one, create it now.
{{< / alert >}}

First we need to [create a release](https://developer.github.com/v3/repos/releases/#create-a-release) for the tag:
...
This performs as `POST` request to the GitHub API. You need to provide three informations for this to work: a token for authorization, your username on GitHub and the name of the repository as url. The last line provides the content for the release. The `tag_name` must match the name of the tag created the step above. `name` and `body` specify the title and the description of the release.
...
We can now use the `upload_url` to upload the three assets to the newly created release:
...
In the `Content-Type` header we provide the mime type of the assets. (Hint: You can get the mime type with `file -b --mime-type /path/to/file`). The url to call is the `upload_url` we got as response with an additional `name` parameter for the filename.
...
To make our lives easier we will automate all of this with Drone in the next section.

# Continuous Integration

In this section we configure [Drone](https://drone.io), an Open Source continuous integration system, to automatically build our project for multiple architectures and publish it every time we create a tag on GitHub. You can install Drone on your own servers, or you can use the free Cloud offering.
...
Defines the Pipeline commands executed inside the Docker container. The commands to execute are the same we have used to build our project. All assets are copied into the newly created assets folder.
...
The final step is to automatically build the project and create a GitHub release if we have created a git tag. In order to use the GitHub API we need to provide our credentials.

## Secrets

For security reasons we do not want to store our credentials in our yaml. Instead you can securely store your secrets in the Drone database, adding your GitHub token via the repository settings screen.

![Manage Secret](/screenshots/github-token-secret.jpg)

## Settings

Drone has a robust plugin system, including a plugin to create GitHub releases. Add a new step to the pipeline and configure the GitHub plugin.
...
Provides the GitHub API token used to authenticate, source from the named secret.
...
This attribute defines the trigger of this step. In this case the step should only be executed when the build reason was a tagged commit.
